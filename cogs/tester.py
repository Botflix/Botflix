...
class Tester(commands.Cog):
    ...
    async def 생성(self, ctx, name=None):
        ...
        if name in self.data[ctx.author.id]:
            ...
            if msg2.author == self.bot.user: return
            elif msg2.content.lower() == "n": return await ctx.send("취소되었습니다.")
            elif msg2.content.lower() == "y":
                del self.data[ctx.author.id][msg.content]
                self.savedata()
                return
        await ctx.send("봇의 Prefix (접두사) 를 입력해주세요. 여러 개라면 `|` 으로 구분해주세요.\n예시 : `!|<@!123456789012345678>|?`")
        msg3 = await waitmsg(self.bot, ctx)
        if msg3.author == self.bot.user: return
        prefixes = msg3.content.split("|")
        await ctx.send("봇을 Private (비공개) 으로 할까요? 비공개로 하면 주인만 사용 가능하고 봇 리스트에 봇이 나오지 않습니다. `Y/N` 으로 대답해주세요.")
        msg2 = await waityn(self.bot, ctx)
        if msg2.author == self.bot.user: return
        ...
        async def check_img():
            ...
            if msg5.content == "없음": return None
            elif url_Tester.scheme == "https" or url_Tester.scheme == "http": return msg5.content
            await ctx.send("올바른 링크 혹은 `없음`을 적어주시기 바랍니다.")
            await check_img()
        avatar = await check_img()

        if not ( ctx.author.id in self.data.keys() ): self.data[ctx.author.id] = dict()
    ...
    async def 프사변경(self, ctx, name, avatar):
        if self.botOwner(ctx, name):
            ...
            if url_Tester.scheme == "https" or url_Tester.scheme == "http":
                self.data[ctx.author.id][name]["avatar"] = avatar
            else: await ctx.send("프로필 사진 링크를 보내주세요.")
            self.savedata()
            await ctx.send("성공!")
        else:
            await ctx.send(f"당신은 {name}의 주인이 아닙니다!")
    ...
    async def 봇정보(self, ctx, name):
        ...
        if data['cmds'] != {}: embed.add_field(name="커맨드", value=f"```{', '.join(list(data['cmds'].keys()))}```")
        else: embed.add_field(name="커맨드",value="없음")
        if data["avatar"] is not None: embed.set_thumbnail(url=data["avatar"])
            ...
            for name in self.data[authId]:
                ...
                for prefix in botDict["prefixes"]:
                    if msg.startswith(prefix):
                        cmd = msg[len(prefix):]
                        if ((not botDict["public"]) and authId == message.author.id) or botDict["public"]:
                            if cmd in botDict["cmds"]:
                                ...
                                if not dd:
                                    if message.channel.permissions_for(message.guild.me) >= discord.Permissions(
                                            manage_webhooks=True):
                                        if len(await message.channel.webhooks()) > 0:
                                            webhookurl = (await message.channel.webhooks())[0].url
                                            await send(
                                                webhookurl,
                                                name,
                                                botDict["avatar"],
                                            ...
                                            webhook = await message.channel.create_webhook(
                                                name="Botflix Webhook", reason="봇플릭스로 만든 봇을 테스트 가능하게 하는 웹훅입니다."
                                            )
                                            await send(
                                                webhook.url,
                                                name,
                                        ...
                                        await message.channel.send("이 채널에서 웹훅 권한이 없네요. `b!웹훅등록 링크`로 등록해주세요.")
                                        break
